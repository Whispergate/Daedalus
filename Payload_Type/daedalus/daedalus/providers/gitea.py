...
logger = logging.getLogger("daedalus.gitea")
...
class GiteaProvider(CIProvider):
    """Gitea Acts runner provider.

    Gitea's Actions API is compatible with Forgejo's (both derive from the
    same Gitea codebase). This provider exists as a distinct entry so users
    can set GITEA_* env vars separately and the provider name is clear in
    logs and tags. The API surface is identical to Forgejo Actions.
    """
    ...
    async def trigger_build(
        ...
        async with self._client() as client:
            ...
            run_id = await self._find_latest_run(client, owner, repo)
        ...
        return BuildResult(
            ...
            url=(
                f"{self.base_url}/{owner}/{repo}/actions/runs/{run_id}"
                if run_id else ""
            ),
        )
            ...
            if r.status_code == 200:
                ...
                for run in runs:
                    if run.get("status") in (
                        "queued", "waiting", "running", "in_progress",
                    ):
                        return str(run["id"])
                if runs:
    ...
    async def get_build_status(self, job: str, build_id: str) -> BuildResult:
        ...
        async with self._client() as client:
            ...
            if r.status_code != 200:
                return BuildResult(
                    provider=self.name, build_id=build_id,
    ...
    async def download_artifact(
        self, job: str, build_id: str, artifact_name: str,
    ) -> bytes:
        owner, repo = self._resolve_repo(job)

        async with self._client(timeout=120) as client:
            list_url = self._api(
                f"/repos/{owner}/{repo}/actions/runs/{build_id}/artifacts"
            )
            ...
            if not target:
                raise FileNotFoundError(
                    f"No artifact {artifact_name!r} in run {build_id}"
                )
    ...
    async def get_build_log(
        self, job: str, build_id: str, tail: int = 100,
    ) -> str:
        owner, repo = self._resolve_repo(job)
        url = self._api(
            f"/repos/{owner}/{repo}/actions/runs/{build_id}/logs"
        )
